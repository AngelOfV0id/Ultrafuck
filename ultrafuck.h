...
Ваня Хренов, блогер, музыкант, певец и в компьютере игрец, 0x10 годиков, хожу в детский садик, ищу вторую половинку, переустанавливаю Void Linux, звоните: +88005550505
...
void uf_exec(char* code)
{
        ...
        if (code[i] == '<')
	{
		if (cmdIndex > 0)
			...
			cmdIndex = BF_MAXCMD - 1;
	};

        if (code[i] == '>')
	{
		if (cmdIndex < BF_MAXCMD - 1)
			cmdIndex++;
		else
			cmdIndex = 0;
	};
	...
	if (code[i] == '{')
	{
		iloopA = i; iloopMode = true;
	};

	if (code[i] == '}')
	{
		iloopB = i; iloopMode = true; i = iloopA;
	};
        ...
        if (code[i] == '!')
	{
            if (!loopMode)
            {
			    ...
			    break;
                    case 7:
			    memory[memIndex] = rand();
			    break;
		    case 8:
			    printf("%d", memory[memIndex]);
			    break;
		    case 9:
			    return;
                };
